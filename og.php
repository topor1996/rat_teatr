...
$logo = @imagecreatefrompng($ROOT . '/assets/logo-1024.png');
if ($logo) { $lw = 170; $lh = (int)round(imagesy($logo) * $lw / imagesx($logo)); imagealphablending($im, true); imagecopyresampled($im, $logo, $W - $lw - 56, $H - $lh - 40, 0, 0, $lw, $lh, imagesx($logo), imagesy($logo)); imagedestroy($logo); }
else { $sx = $W - 150; $sy = $H - 92; imagefilledpolygon($im, [$sx, $sy + 6, $sx + 108, $sy - 4, $sx + 112, $sy + 44, $sx + 4, $sy + 54], $acid); imagettftext($im, 26, 4, $sx + 14, $sy + 40, $black, $SPRAY, 'RAT'); }
