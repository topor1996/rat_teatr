...
function sanitize(array $in): array {
  ...
  return [
    'theatre' => ['name' => str($th['name'] ?? '', 100), 'tagline' => str($th['tagline'] ?? '', 200), 'about' => str($th['about'] ?? '', 3000), 'pressText' => str($th['pressText'] ?? '', 6000), 'pressLead' => str($th['pressLead'] ?? '', 400), 'pressContact' => str($th['pressContact'] ?? '', 300), 'pressCredit' => str($th['pressCredit'] ?? '', 120), 'pressHide' => array_values(array_filter(array_map(fn($x) => img(is_string($x) ? $x : ''), array_slice(is_array($th['pressHide'] ?? null) ? $th['pressHide'] : [], 0, 500)))), 'venue' => str($th['venue'] ?? '', 120),
